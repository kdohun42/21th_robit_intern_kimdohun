...
#define OPERATOR_IP "172.100.5.97"
#define ROBOT_IP    "172.100.2.38"
#define TEXT_PORT   50000
    ...
    , ui(new Ui::MainWindow)
{
    ...
    // bool result = text_socket->bind(QHostAddress::LocalHost, TEXT_PORT);

    if(text_socket->bind(QHostAddress::AnyIPv4, TEXT_PORT))
    {
        qDebug() << "UDP bind 성공";
    ...
    {
        qDebug() << "UDP bind 실패:";
        qDebug() << text_socket->errorString();
    }
...
void MainWindow::udp_read()
{
    while(text_socket->hasPendingDatagrams())
    {
        ...
        ui->textEdit_2->append("상대 : " + text);


        qDebug() << "받은 데이터:" << data;
        qDebug() << "보낸 IP:" << senderAddress;
        qDebug() << "보낸 Port:" << senderPort;
    }
}


void MainWindow::udp_write(
...
void MainWindow::on_btnEnter_clicked()
{
    ...
    // 내가 보낸 메시지는 내 채팅창에만 직접 출력
    ui->textEdit->setAlignment(Qt::AlignRight);
    ui->textEdit->append(QString::fromUtf8(data) + " : 나");

    // 상대 컴퓨터 IP로 전송
    text_socket->writeDatagram(
        data,
        QHostAddress(ROBOT_IP),
        TEXT_PORT
        );
